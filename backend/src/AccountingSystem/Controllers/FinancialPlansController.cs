...
namespace AccountingSystem.Controllers.FinancialPlanning;
...
public class FinancialPlansController : ControllerBase
{
    ...
    public FinancialPlansController(IFinancialPlanService service)
    {
        _service = service;
    }
    ...
    {
        var list = await _service.ListAsync(companyId, ct);
        return Ok(list);
    }
    ...
    {
        var plan = await _service.GetAsync(companyId, id, ct);
    ...
    public async Task<IActionResult> Create([FromQuery] Guid companyId, [FromBody] CreateFinancialPlanInput input, CancellationToken ct = default)
    {
        ...
        var created = await _service.CreateAsync(companyId, input, userId, ct);
    ...
    public async Task<IActionResult> Update(Guid id, [FromQuery] Guid companyId, [FromBody] UpdateFinancialPlanInput input, CancellationToken ct = default)
    {
        ...
        var userId = User?.Identity?.Name ?? "system";
        var updated = await _service.UpdateAsync(companyId, input, userId, ct);
        return Ok(updated);
    }
    ...
    [Authorize(Roles = "Admin,Contabile")]
    public async Task<IActionResult> GenerateForecasts(Guid id, [FromQuery] Guid companyId, [FromQuery] int monthsAhead = 6, [FromQuery] double growthFactor = 1.0, [FromQuery] int historicalMonths = 12, CancellationToken ct = default)
    {
        var userId = User?.Identity?.Name ?? "system";
        var forecasts = await _service.GenerateForecastsAsync(companyId, id, monthsAhead, growthFactor, historicalMonths, userId, ct);
        return Ok(forecasts);
    }
